...
public class CreateStructureFromFilenamesStepPlugin implements IStepPluginVersion2 {
	...
	public PluginReturnValue run() {
		...
		if (!successful) {
			Helper.addMessageToProcessJournal(step.getProcessId(), LogType.ERROR, "CreateStructureFromFilenames plugin encountered an error while processing filenames and updating the document structure.");
			return PluginReturnValue.ERROR;
		} else {
			Helper.addMessageToProcessJournal(step.getProcessId(), LogType.INFO, "CreateStructureFromFilenames plugin successfully processed filenames and updated the document structure.");
		}
