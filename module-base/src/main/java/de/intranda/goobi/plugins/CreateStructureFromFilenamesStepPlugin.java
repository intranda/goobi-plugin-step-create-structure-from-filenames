...
public class CreateStructureFromFilenamesStepPlugin implements IStepPluginVersion2 {
	...
	public PluginReturnValue run() {
		...
		try {
			...
			// Get the names of the folders and sort them in a list and print the list on
			// the console
			...
			for (String fileName : fileList) {
				...
				if (underscoreIndex == -1) {
					...
					if (treeMap.containsKey(modifiedFileName)) {
						// If it exists, add the original fileName to the list associated with
						// modifiedFileName
			...
			for (String key : treeMap.keySet()) {
				...
				for (String fileName : associatedFiles) {
					...
					cf.setMimetype("image/tiff"); // Der MIME-Typ sollte auf das tatsächliche Format abgestimmt sein
